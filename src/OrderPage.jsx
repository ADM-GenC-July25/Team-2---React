...
function OrderPage() {
  ...
  const handleUpdateOrder = () => {
    ...
    setUpdateOrderItems(orderData?.orderItems?.map(item => ({
      id: item.id,
      itemId: item.itemId,
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (actionType === 'update') {
        ...
        endpoint = `/orders/delete/${formData.orderId}?${queryParams.toString()}`;
        requestOptions = {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
        ...
        <div className="order-content">
              ...
              {orderData.orderItems.map((orderItem, index) => (
                <div key={orderItem.id || index} className="order-item">
                  <div className="item-image">
                    <img 
                      src={orderItem.item.img || '/images/placeholder.jpg'} 
                  ...
                  <div className="item-details">
                    ...
                    <div className="item-meta">
                      <span className="category">{orderItem.item.category}</span>
                      {orderItem.item.spiceLevel && (
                        <span className="spice-level">🌶️ {orderItem.item.spiceLevel}</span>
                      )}
                    </div>
                    {orderItem.customerOrder && (
                      <p className="special-instructions">
                        <strong>Special Instructions:</strong> {orderItem.customerOrder}
                      </p>
                    )}
                    {orderItem.item.allergens && orderItem.item.allergens.length > 0 && (
                      <p className="allergens">
                        <strong>Allergens:</strong> {orderItem.item.allergens.join(', ')}
                      </p>
                    )}
                  </div>
                  <div className="item-pricing">
                    <div className="quantity">Qty: {orderItem.quantity}</div>
                    <div className="price">${formatPrice(orderItem.item.price)}</div>
          ...
          <div className="order-totals">
            ...
            <div className="totals-breakdown">
              ...
              <div className="payment-status">
                <span className={`status ${orderData.payed ? 'paid' : 'pending'}`}>
          ...
          <div className="pickup-info">
            ...
            <div className="pickup-details">
              <p><strong>Location:</strong> Asteroid Belt, Milky Way Galaxy</p>
              <p><strong>Estimated Pickup Time:</strong> 20 minutes</p>
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleFormSubmit} className="order-form">
              ...
              {orderData?.orderItems && orderData.orderItems.length > 0 && (
                <div className="form-group">
                  <label>
                    {actionType === 'update' ? 'Order Items:' : 'Items to be deleted:'}
                  </label>
                    ...
                    {actionType === 'update' ? (
                      ...
                      orderData.orderItems.map((orderItem) => (
                        <div key={orderItem.itemId} className="edit-item">
                          <div className="item-info">
                            <span className="item-name">{orderItem.item?.name || 'Unknown Item'}</span>
                            <span className="item-price">${(orderItem.item?.price || 0).toFixed(2)}</span>
                          </div>
                          <div className="quantity-control">
                            <span className="quantity-display">Qty: {orderItem.quantity}</span>
                          </div>
                        </div>
                      ))
                    )}
              ...
              <div className="form-actions">
                <button 
                  ...
                  className={actionType === 'update' ? 'btn-update' : 'btn-cancel'}
                >
                  {isProcessing 
                    ? 'Processing...' 
                    : actionType === 'update' ? 'Update Order' : 'Delete Order'}
                </button>
                <button 
                  type="button" 
